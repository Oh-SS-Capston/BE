...
public class OllamaConfig {
     ...
     * 생성 난수 시드.
     *
     * <p>고정하면 같은 프롬프트에 같은 출력이 나온다. 설정을 바꿔가며 품질을 비교할 때
     * 이게 없으면 "설정 때문에 달라진 것"과 "모델이 원래 실행마다 다른 것"을 구분할 수 없다.
     * 실제로 시나리오 품질 비교 실험이 이 구분 불가 때문에 결론을 못 낸 적이 있어
     * 기본을 고정값으로 둔다(파이프라인의 재현성 목표와도 맞다).</p>
     *
     * <p>음수면 options에서 생략해 Ollama 기본 동작(호출마다 임의 시드)으로 되돌린다.</p>
     */
    private int seed = 0;
