...
/**
 * Endpoint/Event/SPI GraphStore shadow 후보와 Extraction Relation을 비교한다.
 */
public final class EndpointEventSpiShadowParityAnalyzer {
    ...
    private EndpointEventSpiShadowParityAnalyzer() {
    }
    ...
    public static ObservationPromotionCandidateParityReport compare(
        ...
        ShadowFactsIndex safeIndex = factsIndex == null
                ...
                : factsIndex;
        // 공통 relation 인덱스에서 대상 kind만 꺼내 비교해 전체 relations 반복 순회를 줄인다.
        // 성능 최적화: relationByKeyForKinds가 이미 remove 가능한 새 Map을 반환하므로 추가 복사를 생략한다.
        // parity 비교 로직은 동일하게 extractionByKey.remove(key)를 사용하고, 중간 heap 사용량만 줄인다.
        ...
        List<ObservationPromotionShadowCandidate> candidates =
                generated == null ? List.of() : generated.candidates();
        List<ObservationPromotionCandidateParityIssue> issues = new ArrayList<>();

        for (ObservationPromotionShadowCandidate candidate : candidates) {
            String key = candidate.relationKey();
            NormalizedRelationFact extraction = extractionByKey.remove(key);

            if (extraction == null) {
                issues.add(new ObservationPromotionCandidateParityIssue(
                        key,
                        ObservationPromotionCandidateParityStatus.MISSING_EXTRACTION_RELATION,
                        candidate.observationIndex(),
                        candidate.observationKind(),
                        List.of("No Extraction relation exists for the generated relation key")
                ));
                continue;
            }

            List<String> reasons = metadataDifferences(candidate.relation(), extraction, mapper);
            issues.add(new ObservationPromotionCandidateParityIssue(
                    key,
                    reasons.isEmpty()
        ...
        for (Map.Entry<String, NormalizedRelationFact> entry : extractionByKey.entrySet()) {
            issues.add(new ObservationPromotionCandidateParityIssue(
                    entry.getKey(),
                    ObservationPromotionCandidateParityStatus.EXTRACTION_ONLY,
                    null,
                    null,
                    List.of("Extraction produced a target relation that GraphStore shadow generation did not reproduce")
            ));
        }
    ...
    private static List<String> metadataDifferences(
        ...
        compare(reasons, "origin", normalizeCode(generated.origin()), normalizeCode(extraction.origin()));
        compare(reasons, "derivation", normalizeCode(generated.derivation()), normalizeCode(extraction.derivation()));
        compare(reasons, "resolution.status", normalizeCode(generated.resolutionStatus()), normalizeCode(extraction.resolutionStatus()));
        compare(reasons, "resolution.reason", generated.resolutionReason(), extraction.resolutionReason());
        compare(reasons, "call_site_line", generated.callSiteLine(), extraction.callSiteLine());

        if (!sameDecimal(generated.confidenceHint(), extraction.confidenceHint())) {
            reasons.add("confidence_hint expected=" + generated.confidenceHint()
                    + ", actual=" + extraction.confidenceHint());
        }

        if (!Objects.equals(generated.evidenceIds(), extraction.evidenceIds())) {
            reasons.add("evidence_ids expected=" + generated.evidenceIds()
                    + ", actual=" + extraction.evidenceIds());
        }

        JsonNode generatedAttrs = mapper.valueToTree(generated.attrs());
        JsonNode extractionAttrs = mapper.valueToTree(extraction.attrs());

        if (!Objects.equals(generatedAttrs, extractionAttrs)) {
            reasons.add("attrs expected=" + generatedAttrs + ", actual=" + extractionAttrs);
        }

        return List.copyOf(reasons);
    }

    private static void compare(List<String> reasons, String field, Object expected, Object actual) {
